...
| C4 | **M2 was never verified in a browser.** Chromium is missing system libs (`libnss3`, `libnspr4`, `libasound2t64`), which also broke chrome-devtools-mcp. Rendering, cell flash, scroll smoothness and console cleanliness are all unverified. | QA | Re-run the 7-point M2 checklist once the libs are installed. Confined to `grid-viewport` + app page; the worker pipeline is covered by unit and live-AMPS integration tests. |
